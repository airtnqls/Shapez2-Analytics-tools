...
class HybridTracer:
    """하이브리드 도형 분석을 위한 트레이서 클래스"""
    
    @staticmethod
    def hybrid(shape: Shape, claw: bool = False) -> Tuple[Shape, Shape]:
        """
        ...
        while current_layer < len(shape.layers):
            ...
            unstable_coords = HybridTracer._find_unstable_at_layer(temp_shape, current_layer, reversed_mask)
        ...
        for current_layer in range(len(shape.layers)):
            for q in range(4):
                ...
                temp_shape = Shape(temp_layers)
                unstable_coords = HybridTracer._find_unstable_at_layer(temp_shape, current_layer, mask)
                ...
                if coord in unstable_coords:
                    ...
                    for nq in range(4):
                        if HybridTracer._is_adjacent(q, nq):
