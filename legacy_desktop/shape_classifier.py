...
# Compatibility façade for the original widgets.  Classification authority is
# the supplied ZIP worker; the historical classifier above is retained only so
# old imports and ShapeType presentation values remain available to the GUI.
def analyze_shape(shape: str, shape_obj=None, skip: bool = False) -> tuple[str, str]:
    from zip_backend import classify_shape
    return classify_shape(shape)


def get_edge_pillars(shape: str) -> list[str]:
    """Return UI search columns after ZIP structural simplification."""
    from zip_backend import operate_shape
    outputs = operate_shape("simplify", shape).get("outputs", [])
    simplified = outputs[0] if outputs else shape
    layers = [layer.ljust(4, "-")[:4] for layer in simplified.split(":") if layer]
    return [":".join(layer[column] for layer in layers) for column in range(4)]
