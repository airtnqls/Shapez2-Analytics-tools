...
class Shape:
    ...
    def classifier(self) -> tuple[str, str]:
        ...
        # The original GUI stays intact; classification is never allowed to
        # fall back to the retired Python analyzer.
        from zip_backend import classify_shape
        return classify_shape(shape_string)
...
def _zip_shape_result(operation: str, shape_a: Shape, shape_b: Shape | None = None, **kwargs) -> list[Shape]:
    from zip_backend import operate_shape
    cap = max(getattr(shape_a, 'max_layers', Shape.MAX_LAYERS), len(shape_a.layers))
    response = operate_shape(operation, repr(shape_a), repr(shape_b) if shape_b else "", cap, **kwargs)
    return [Shape.from_string(code) for code in response.get("outputs", [])]


def _zip_destroy_half(self): return _zip_shape_result("destroy_half", self)[0]
def _zip_apply_physics(self, debug=False):
    value = _zip_shape_result("apply_physics", self)[0]
    return (value, "ZIP physics worker") if debug else value
def _zip_push_pin(self): return _zip_shape_result("push_pin", self)[0]
def _zip_crystal(self, color_str): return _zip_shape_result("crystal_generator", self, crystal_color=color_str)[0]
def _zip_rotate(self, clockwise=True): return _zip_shape_result("rotate_cw" if clockwise else "rotate_ccw", self)[0]
def _zip_rotate_180(self): return _zip_shape_result("rotate_180", self)[0]
def _zip_mirror(self): return _zip_shape_result("mirror", self)[0]
def _zip_paint(self, color_str): return _zip_shape_result("paint", self, paint_color=color_str)[0]
def _zip_half_cutter(self): return tuple(_zip_shape_result("half_cutter", self))
def _zip_simple_cutter(self, horizontal=False): return tuple(_zip_shape_result("simple_cutter", self))
def _zip_quad_cutter(self): return tuple(_zip_shape_result("quad_cutter", self))
def _zip_stack(bottom, top): return _zip_shape_result("stack", bottom, top)[0]
def _zip_swap(shape_a, shape_b): return tuple(_zip_shape_result("swap", shape_a, shape_b))
def _zip_classifier(self):
    from zip_backend import classify_shape
    return classify_shape(repr(self))


# Preserve the original GUI surface while making the supplied ZIP worker the
# single source of truth for every building operation.
Shape.destroy_half = _zip_destroy_half
Shape.apply_physics = _zip_apply_physics
Shape.push_pin = _zip_push_pin
Shape.crystal_generator = _zip_crystal
Shape.rotate = _zip_rotate
Shape.rotate_180 = _zip_rotate_180
Shape.mirror = _zip_mirror
Shape.paint = _zip_paint
Shape.half_cutter = _zip_half_cutter
Shape.simple_cutter = _zip_simple_cutter
Shape.quad_cutter = _zip_quad_cutter
Shape.stack = staticmethod(_zip_stack)
Shape.swap = staticmethod(_zip_swap)
Shape.classifier = _zip_classifier
