...
# ZIP-worker overrides -----------------------------------------------------
# The PyQt buttons keep their historical names and positions, but their
# results come from the supplied ZIP physics/proof contract rather than the
# retired Python tracers above.
def _zip_single(operation: str, shape_code: str) -> str:
    from zip_backend import operate_shape
    outputs = operate_shape(operation, shape_code).get("outputs", [])
    return outputs[0] if outputs else ""


def simplify_shape(shape_code: str) -> str: return _zip_single("simplify", shape_code)
def detail_shape(shape_code: str) -> str: return _zip_single("detail", shape_code)
def corner_1q_shape(shape_code: str) -> str: return _zip_single("corner_1q", shape_code)
def reverse_shape(shape_code: str) -> str: return _zip_single("reverse", shape_code)
def mirror_shape_for_gui(shape_code: str) -> str: return _zip_single("mirror", shape_code)
def cornerize_shape(shape_code: str) -> str: return _zip_single("cornerize", shape_code)


def _zip_trace(shape_code: str) -> List[str]:
    from zip_backend import trace_inputs
    _, inputs = trace_inputs(shape_code)
    return inputs


def corner_shape_for_gui(shape_code: str) -> str:
    values = _zip_trace(shape_code)
...
def claw_shape_for_gui(shape_code: str, logger=None) -> str:
    values = _zip_trace(shape_code)
    if logger:
        logger(f"ZIP proof backpointer 입력 {len(values)}개")
    return values[0] if values else ""


def hybrid_shape(shape_code: str) -> List[str]: return _zip_trace(shape_code)
def claw_hybrid_shape(shape_code: str) -> List[str]: return _zip_trace(shape_code)
