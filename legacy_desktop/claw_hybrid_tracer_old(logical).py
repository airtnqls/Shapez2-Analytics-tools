...
def claw_hybrid(shape: Shape) -> Tuple[Shape, Shape]:
    """
    하이브리드 함수: 입력을 마스크 기반으로 두 부분으로 분리합니다. 논리적 방식으로.
    ...
    # 3. 클로시 크리스탈 주변 마스크 0으로 설정 및 가장 높은 크리스탈 찾기
    ...
    # 2. 0층의 P와 S는 highest_crystal_quadrant의 반대 사분면(+2)만 마스크 0으로 설정
    if highest_crystal_quadrant != -1:
        opposite_quadrant = (highest_crystal_quadrant + 2) % 4
        piece = shape._get_piece(0, opposite_quadrant)
        if piece and (piece.shape == 'P' or piece.shape == 'S'):
            mask[(0, opposite_quadrant)] = 0
    
    # 4. 각 사분면의 가장 높은 크리스탈을 찾아 마스크 처리
    for q in range(4):
        highest_crystal_layer_in_quad = -1
        for l in range(len(shape.layers) - 1, -1, -1):  # 위에서부터 탐색
            piece = shape._get_piece(l, q)
            if piece and piece.shape == 'c':
                highest_crystal_layer_in_quad = l
                break
        
        # 각 크리스탈과 그 아래는 모두 마스크 영역을 0로 만듭니다
        if highest_crystal_layer_in_quad >= 0:
            for l in range(highest_crystal_layer_in_quad + 1):  # 해당 층과 그 아래 모든 층
                mask[(l, q)] = 0
