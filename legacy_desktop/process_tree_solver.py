...
class ProcessNode:
    ...
    def __init__(self, shape_code: str, operation: str = "", node_id: str = None, input_ids: List[str] = None):
        ...
        self.node_kind = "shape"
        self.operation_code = ""
        self.display_label = operation
        self.metadata = {}
        self.shape_obj = None
...
def _solve_with_zip_backend(self, target_shape_code: str) -> Optional[ProcessNode]:
    """Build the legacy ProcessNode view from the ZIP proof DAG.

    The rendering/layout methods below remain the original GUI implementation;
    only the broken heuristic/omission-producing solver is replaced.
    """
    from zip_backend import analyze_shape as analyze_zip_shape

    self.nodes_map.clear()
    self.next_node_id = 0
    result = analyze_zip_shape(target_shape_code, mode="proof")
    graph = result.get("proof") or {}
    graph_nodes = graph.get("nodes") or []
    graph_edges = graph.get("edges") or []
    root_id = str(graph.get("rootId") or "")
    by_id = {str(item.get("id")): item for item in graph_nodes}
    ...
    while pending:
        current = pending.pop()
        if not current or current in selected:
            continue
        selected.add(current)
        pending.extend(str(edge.get("source")) for edge in incoming.get(current, []))
    ...
    # Preserve every graph vertex.  The old adapter folded operation vertices
    # into shape labels, which made the GUI appear to change shapes without
    # showing CUT/STACK/SWAP/etc.  The legacy view now alternates explicit
    # shape -> operation-icon -> input-shape nodes.
    for item in graph_nodes:
        ...
        input_ids = [
            str(edge.get("source")) for edge in incoming.get(node_id, [])
            if str(edge.get("source")) in selected
        ]
        operation_code = str(item.get("operation") or "")
        label = operation_labels.get(operation_code, str(item.get("label") or operation_code or "판정 근거"))
        shape_code = str(item.get("code") or "") if kind == "shape" else ""
        node = ProcessNode(shape_code, label if kind != "shape" else "", node_id, input_ids)
        node.node_kind = kind
        node.operation_code = operation_code
        node.display_label = label
        node.metadata = dict(item.get("metadata") or {})
        if kind != "shape":
            node.classification = ""
            node.classification_reason = ""
        self.nodes_map[node_id] = node

    root = self.nodes_map.get(root_id)
    if root is None:
        root = ProcessNode(target_shape_code, self.IMPOSSIBLE_OPERATION, "zip-root")
        self.nodes_map[root.node_id] = root
    root.classification = str(result.get("shapeType") or "")
    root.classification_reason = str(result.get("reason") or "")
    if result.get("verdict") == "IMPOSSIBLE":
        root.operation = self.IMPOSSIBLE_OPERATION
    return root


# Replace only the backend entry point; all original GUI features and tree
# layout/rendering APIs remain in service.
ProcessTreeSolver.solve_process_tree = _solve_with_zip_backend


def _zip_dag_levels(self, root_node: ProcessNode) -> List[List[ProcessNode]]:
    """Longest-path ranks for a root-to-input proof DAG."""
    if not root_node:
        return []
    ranks = {root_node.node_id: 0}
    queue = [root_node.node_id]
    relaxations = 0
    limit = max(1, len(self.nodes_map) * len(self.nodes_map))
    while queue and relaxations < limit:
        parent_id = queue.pop(0)
        parent = self.nodes_map.get(parent_id)
        if not parent:
            continue
        next_rank = ranks[parent_id] + 1
        for child_id in parent.input_ids:
            if child_id not in self.nodes_map:
                continue
            if next_rank > ranks.get(child_id, -1):
                ranks[child_id] = next_rank
                queue.append(child_id)
            relaxations += 1
    levels = [[] for _ in range(max(ranks.values(), default=0) + 1)]
...
def _zip_dag_positions(self, root_node: ProcessNode, node_sizes: Dict) -> Dict[ProcessNode, Tuple[float, float]]:
    """Sugiyama-style rank ordering with barycentric crossing reduction."""
    levels = self.get_tree_levels(root_node)
    if not levels:
        return {}
    parent_map = self._build_parent_map()
    order = {node.node_id: index for level in levels for index, node in enumerate(level)}

    # Alternate downward/upward barycentric sweeps.  This is deliberately
    # deterministic so the same proof does not jump between generations.
    for _ in range(5):
        for level_index in range(1, len(levels)):
            previous = {node.node_id: index for index, node in enumerate(levels[level_index - 1])}
            levels[level_index].sort(key=lambda node: (
                sum(previous[parent] for parent in parent_map.get(node.node_id, []) if parent in previous)
                / max(1, sum(parent in previous for parent in parent_map.get(node.node_id, []))),
                order.get(node.node_id, 0),
            ))
        for level_index in range(len(levels) - 2, -1, -1):
            following = {node.node_id: index for index, node in enumerate(levels[level_index + 1])}
            levels[level_index].sort(key=lambda node: (
                sum(following[child] for child in node.input_ids if child in following)
                / max(1, sum(child in following for child in node.input_ids)),
                order.get(node.node_id, 0),
            ))

    positions: Dict[ProcessNode, Tuple[float, float]] = {}
    y = 0.0
    horizontal_gap = 72.0
    vertical_gap = 68.0
    for level in levels:
        if not level:
            continue
        widths = [float(node_sizes.get(node, (120, 60))[0]) for node in level]
        total_width = sum(widths) + horizontal_gap * max(0, len(level) - 1)
        x = -total_width / 2.0
        max_height = max(float(node_sizes.get(node, (120, 60))[1]) for node in level)
...
ProcessTreeSolver.get_tree_levels = _zip_dag_levels
ProcessTreeSolver.calculate_tree_positions = _zip_dag_positions
