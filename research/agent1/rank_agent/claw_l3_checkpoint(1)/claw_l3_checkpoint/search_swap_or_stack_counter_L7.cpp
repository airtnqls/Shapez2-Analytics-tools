...
bool topmost_has_crystal(const Grid&g){
 for(int r=L-1;r>=0;r--){bool any=false,cr=false;for(int q=0;q<4;q++){any|=occ(g[r][q]);cr|=g[r][q].k==C;}if(any)return cr;}return false;
}
uint64_t rngstate=0x9e3779b97f4a7c15ULL;
uint64_t rng64(){rngstate^=rngstate<<7;rngstate^=rngstate>>9;return rngstate;}
int main(){
 vector<Half> both, anyfall;
 const uint64_t total=1ull<<(4*L);uint64_t stable_halves=0;
 for(uint64_t x=0;x<total;x++){
 ...
 cerr<<"halves stable="<<stable_halves<<" any="<<anyfall.size()<<" both="<<both.size()<<"\n";
 const uint64_t N=100000000ULL;uint64_t compatible=0,three=0,cr=0,bothunst=0,toptcr=0;
 for(uint64_t i=0;i<N;i++){
   const auto&A=both[rng64()%both.size()];const auto&B=anyfall[rng64()%anyfall.size()];
   if((A.c1&B.c0)||(A.c0&B.c1))continue;compatible++;
   Grid full{};for(int r=0;r<L;r++){full[r][0].k=A.a[r][0];full[r][1].k=A.a[r][1];full[r][2].k=B.a[r][0];full[r][3].k=B.a[r][1];}
   Grid out{};auto fm=push_fallmask(full,out);if(__builtin_popcount((unsigned)fm)<=2)continue;three++;if(!has_crystal(out))continue;cr++;
   if(cut_axis_stable(out,0)||cut_axis_stable(out,1))continue;bothunst++;
   if(!topmost_has_crystal(out))continue;toptcr++;
   cout<<"SWAP_STACK_PROXY_COUNTER pre="<<gs(full)<<" out="<<gs(out)<<" fm="<<(int)fm<<"\n";return 0;
 }
 cout<<"NO_RANDOM_COUNTER L="<<L<<" samples="<<N<<" stable="<<stable_halves<<" any="<<anyfall.size()<<" both="<<both.size()<<" compatible="<<compatible<<" three="<<three<<" crystal="<<cr<<" bothunst="<<bothunst<<" topcr="<<toptcr<<"\n";
}
