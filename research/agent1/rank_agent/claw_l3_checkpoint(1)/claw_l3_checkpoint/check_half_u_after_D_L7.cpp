...
int old_main(){
 ...
 for(uint64_t x=0;x<total;x++){
  ...
  if(fixed_two_eager_bad(p,o,cm,d)){
    cout<<"FIXED_TWO_EAGER_INTER_COUNTER "<<gs(p)<<" -> "<<gs(o)<<" common="<<cm<<" lastd="<<d[0]<<d[1]<<d[2]<<d[3]<<" axes="<<a0<<a1<<"\\n";return 0;
  }
 }
 cout<<"NO_INTER_INTER_COUNTER total="<<total<<" topc="<<topc<<" stable="<<stable<<" cutstable="<<cutstable<<"\\n";
}

uint8_t push_deepmask(const Grid&pre, Grid&out){
 Grid g{};
 for(int q=0;q<4;q++)if(occ(pre[0][q]))g[0][q]={P,-1};
 for(int r=0;r<L;r++)for(int q=0;q<4;q++){g[r+1][q]=pre[r][q];if(occ(g[r+1][q]))g[r+1][q].origin=r;}
 vector<pair<int,int>> seeds;for(int q=0;q<4;q++)if(occ(g[L][q]))seeds.push_back({L,q});
 delete_crystal_components(g,L+1,seeds);for(int q=0;q<4;q++)g[L][q]=Cell{};gravity(g,L);out=g;
 uint8_t m=0;
 for(int q=0;q<4;q++){
   int highest_fixed=-1;for(int r=0;r<L;r++)if(occ(g[r][q])&&g[r][q].origin>=0&&r==g[r][q].origin+1)highest_fixed=r;
   if(highest_fixed<0)continue;
   for(int r=0;r<highest_fixed;r++)if(occ(g[r][q])&&g[r][q].origin>=0&&g[r][q].k!=C&&r<g[r][q].origin+1){m|=1<<q;break;}
 }
 return m;
}
uint8_t pregravity_badmask(const Grid&pre, Grid&gout){
 Grid g{};for(int q=0;q<4;q++)if(occ(pre[0][q]))g[0][q]={P,-1};for(int r=0;r<L;r++)for(int q=0;q<4;q++){g[r+1][q]=pre[r][q];if(occ(g[r+1][q]))g[r+1][q].origin=r;}
 vector<pair<int,int>>seeds;for(int q=0;q<4;q++)if(occ(g[L][q]))seeds.push_back({L,q});delete_crystal_components(g,L+1,seeds);for(int q=0;q<4;q++)g[L][q]=Cell{};
 auto sup=supportmask(g,L);gout=g;for(int r=0;r<L;r++)for(int q=0;q<2;q++)if(g[r][q].k==C&&!(sup&(1u<<(4*r+q))))return 1;return 0;
}

int main(){uint64_t total=1ull<<(4*L),top=0,stable=0,bad=0;
 for(uint64_t x=0;x<total;x++){
  Grid p{};uint64_t y=x;for(int r=0;r<L;r++)for(int q=0;q<2;q++){p[r][q].k=K(y&3);y>>=2;}
  if(p[L-1][0].k!=C&&p[L-1][1].k!=C)continue;top++;
  if(!stable_pre(p))continue;stable++;
  Grid g{};auto m=pregravity_badmask(p,g);if(m){bad++;cout<<"U_HALF_COUNTER "<<gs(p)<<" mid="<<gs(g)<<" mask="<<(int)m<<"\n";return 0;}
 }
 cout<<"NO_U_HALF_COUNTER L="<<L<<" raw="<<total<<" top="<<top<<" stable="<<stable<<" bad="<<bad<<"\n";
}
