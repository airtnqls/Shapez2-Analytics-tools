...
Result SolvePinPush(const std::vector<Row>& target, bool allow_unsupported = true,
  ...
  return SolveOverflow(target, allow_unsupported, require_unsupported, prefer_unsupported, require_swappable, nullptr);
}

Result SolveRawOverflow(const std::vector<Row>& target) {
  return SolveOverflow(target, true, false, false, false, nullptr);
}
...
int main(int argc, char** argv) {
  ...
  if (offset < argc && std::string(argv[offset]) == "--batch") {
    ...
    while (std::getline(std::cin, line)) {
      ...
      const auto result = SolvePinPush(target, !no_unsupported, require_unsupported, prefer_unsupported, require_swappable);
